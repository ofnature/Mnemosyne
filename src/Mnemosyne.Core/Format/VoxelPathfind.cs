...
public class VoxelPathfind
{
    ...
    public List<(ulong voxel, Vector3 p)> FindPath(ulong fromVoxel, ulong toVoxel, Vector3 fromPos, Vector3 toPos, bool useRaycast, bool returnIntermediatePoints, CancellationToken cancel, Vector3? avoidCenter = null, float avoidRadius = 0)
    {
        ...
        Execute(cancel, MaxSteps); // patched for Mnemosyne: budget cap, partial path toward goal on exhaustion
        return BuildPathToVisitedNode(_bestNodeIndex, returnIntermediatePoints);
    }

    // patched for Mnemosyne: search budget. Long open-air hops explore >1M nodes (~9s);
    // capping returns a partial path toward the goal - callers re-query from its end.
    ...
    private List<(ulong voxel, Vector3 p)> FindPathUnavoided(ulong fromVoxel, ulong toVoxel, Vector3 fromPos, Vector3 toPos, bool useRaycast, bool returnIntermediatePoints, CancellationToken cancel)
    {
        ...
        {
            Start(fromVoxel, toVoxel, fromPos, toPos);
            Execute(cancel, MaxSteps); // patched for Mnemosyne: budget cap (see MaxSteps)
            return BuildPathToVisitedNode(_bestNodeIndex, returnIntermediatePoints);
        }
        finally
        {
    ...
    public void Start(ulong fromVoxel, ulong toVoxel, Vector3 fromPos, Vector3 toPos)
    {
        ...
        {
            /* patched for Mnemosyne: no Dalamud logger */
            return;
        }
    ...
    {
        float randomFactor = 0; // patched for Mnemosyne: no Dalamud config
    ...
    // note: heuristic inflation (weighted A*) was tried and measured SLOWER here - the
    // inconsistent heuristic causes mass re-expansion of closed nodes. Keep admissible.
