...
public class NavmeshCustomization
{
	...
	public virtual bool IsFlyingSupported(SceneDefinition definition) => FlyingSupportedResolver?.Invoke(definition.TerritoryID) ?? false; // patched for Mnemosyne: injected (Lumina sheet lookup lives in the builder host)
    public static Func<uint, bool>? FlyingSupportedResolver;
...
public static class NavmeshCustomizationRegistry
{
	...
	static NavmeshCustomizationRegistry()
	{
		...
		foreach (var t in Assembly.GetExecutingAssembly().DefinedTypes.Where(t => t.IsSubclassOf(baseType)))
		{
			...
			if (instance == null)
			{
				Console.WriteLine($"Failed to create instance of customization class {t}"); // patched for Mnemosyne
				continue;
			}
...
public static class CreateParamsExtensions
{
	public static void AddOffMeshConnection(this DtNavMeshCreateParams config, Vector3 ptA, Vector3 ptB, float radius = 0.5f, bool bidirectional = false, int userID = 0)
	{
		...
		if (aInside != bInside)
		{
			Console.WriteLine("off-mesh connection would span two tiles; unsupported"); // patched for Mnemosyne
			Console.WriteLine($"Bounding box of matched tile: {config.bmin} <=> {config.bmax}"); // patched for Mnemosyne
