...
{
	// patched for Mnemosyne: file access injected by the builder host (Lumina sqpack reader)
	...
	public unsafe SceneExtractor(SceneDefinition scene)
	{
		...
		{
			var list = FileReader?.Invoke(terr + "/list.pcb"); // patched for Mnemosyne: injected file access
			if (list is { Length: > 0 })
			{
				fixed (byte* data = &list[0])
				{
	...
	{
		var mesh = new Mesh();
		var f = FileReader?.Invoke(path); // patched for Mnemosyne: injected file access
		if (f is { Length: > 0 })
		{
			fixed (byte* rawData = &f[0])
			{
	...
	private static AABB CalculateSphereBounds(ulong id, ref Matrix4x3 world)
	{
		...
		if (Math.Abs(scale - world.Row1.Length()) > 0.1 || Math.Abs(scale - world.Row2.Length()) > 0.1)
			Console.WriteLine($"Sphere {id:X} has non-uniform scale"); // patched for Mnemosyne
